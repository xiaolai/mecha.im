...
When you run `mecha bot spawn researcher ~/papers`, Mecha:
...
An unqualified name like `researcher` resolves to `researcher@local` — the bot on the current node.
...
Sessions persist across bot restarts. You can list and review them:
...
A workspace is the directory a bot is allowed to access. When you spawn an agent:
...
Tags organize bots into logical groups:
...
mecha bot configure researcher --tags research,nlp
```

Tags power:
- **Discovery** — `mecha bot find --tag dev` lists all dev agents
- **Group addressing** — `+research` targets all research agents
- **ACL rules** — grant permissions to groups by tag

## State Machine

Each bot has a lifecycle state:
...
| `stopped` | Gracefully stopped via `mecha bot stop` |
| `error` | Crashed or failed health check |

Check state with:

```bash
mecha bot ls              # all bots
mecha bot status coder    # single bot detail
```
...
│   ├── identity.json            ← bot Ed25519 public key + node signature
│   ├── bot.key                 ← bot private key (mode 0600)
│   ├── logs/
│   │   ├── stdout.log
│   │   └── stderr.log
│   ├── home/                    ← redirected HOME for this bot
│   │   └── .claude/
...
Each bot's `home/` directory is an isolated Claude Code environment — the `HOME` env var is redirected there so Claude Code reads its own settings, not the host's. See [Sandbox & Security](/features/sandbox#bot-home-directory-isolation) for details.

## Parent-Child Workspaces

When one bot's workspace is a subdirectory of another's, Mecha automatically detects the relationship. For example, if `coder` owns `~/project` and `reviewer` owns `~/project/reviews`, then `mecha bot status reviewer` will show `coder` as the parent. This is purely informational — no permissions are implied.
