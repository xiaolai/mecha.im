...
| `--expose <caps>` | Comma-separated capabilities to expose (`query`, `read_workspace`, `write_workspace`, `execute`, `read_sessions`, `lifecycle`, `bus_publish`, `bus_subscribe`, `bus_queue`, `workflow_run`, `workflow_manage`, `gateway_outbound`, `meta`) | |
...
Gracefully stop one or more bots (SIGTERM).
...
mecha bot stop alice bob charlie
mecha bot stop researcher --force
```

## `mecha bot kill`

Immediately kill one or more bot processes (SIGKILL). Keeps data on disk.
...
Stop and delete one or more bots (config, logs, sessions).
...
| Option | Description | Default |
|--------|-------------|---------|
| `--force` | Force kill instead of graceful stop, bypasses busy-session check | `false` |

```bash
mecha bot remove old-agent
mecha bot remove alice bob --force
```
...
```bash
mecha bot ls [options]
```

| Option | Description | Default |
|--------|-------------|---------|
| `--mesh` | Include bots from all registered nodes | `false` |
...
Show recent activity for a bot.

```bash
mecha bot activity <name> [options]
```
...
| Option | Description |
|--------|-------------|
| `-w, --watch` | Stream activity in real-time |

```bash
mecha bot activity researcher
mecha bot activity researcher --watch
```
