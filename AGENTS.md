...
4. **GUI Last** — Only then add spa components as thin wrappers over shared logic
...
- `.claude/rules/no-gui-without-cli.md` — Blocks GUI-only features in spa
...
│   └── workflow/
├── dev-docs/
│   └── plan/              ← v2 phase plans (orchestration-layers.md)
...
│   ├── docs-guardian/     ← documentation guardian agent
│   └── tdd-guardian/      ← TDD guardian agent
...
`config.json` includes core fields (port, token, workspace, model, tags, expose, sandboxMode, allowNetwork, permissionMode, configVersion, auth, home) plus optional spawn settings for LLM behavior (systemPrompt, appendSystemPrompt, effort, maxBudgetUsd), tool control (allowedTools, disallowedTools, tools), agent identity (agent, agents, addDirs, budgetLimit), MCP (mcpServers, mcpConfigFiles, strictMcpConfig), session behavior (sessionPersistence, disableSlashCommands), permission overrides (dangerouslySkipPermissions, allowDangerouslySkipPermissions), model fallback (fallbackModel), environment (env), and settingSources (defaults to `["project", "user"]` — bots load both project and user settings).
...
- `disallowedTools` and `tools` are mutually exclusive — use `disallowedTools` for subtractive filtering or `tools` for a full override
- `permissionMode: "bypassPermissions"` requires `sandboxMode: "require"` — only sandboxed bots may bypass permission prompts
- `dangerouslySkipPermissions` requires `sandboxMode: "require"` — same safety constraint as permissionMode bypassPermissions
