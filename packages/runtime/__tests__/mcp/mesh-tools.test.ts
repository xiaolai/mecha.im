...
import { handleMeshTool, sanitizeMessage, type MeshOpts, type MeshRouter } from "../../src/mcp/mesh-tools.js";
...
describe("mesh_query", () => {
  ...
  it("proxies through agent when router not available", async () => {
    const opts: MeshOpts = { mechaDir, botName: "coder" };
    const result = await handleMeshTool(opts, "mesh_query", { target: "researcher", message: "hello" });

    // Without a running agent daemon, proxy fails with a connection or auth error
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toMatch(/Mesh query failed/);
  });
});

describe("sanitizeMessage", () => {
  it("strips control characters except newline and tab", () => {
    const input = "hello\x00\x01\x02\x03world\tnext\nline\x1F!";
    const result = sanitizeMessage(input);
    expect(result).toBe("helloworld\tnext\nline!");
  });

  it("passes normal messages through unchanged", () => {
    const input = "Hello, can you search for papers on AI safety?";
  ...
  it("truncates messages over 64KB", () => {
    const input = "A".repeat(65 * 1024);
    const result = sanitizeMessage(input);
    expect(Buffer.byteLength(result, "utf-8")).toBeLessThanOrEqual(64 * 1024);
    expect(Buffer.byteLength(result, "utf-8")).toBe(64 * 1024);
  });

  it("does not truncate messages at exactly 64KB", () => {
    const input = "B".repeat(64 * 1024);
    const result = sanitizeMessage(input);
    expect(result).toBe(input);
  });
});

describe("mesh_query sanitization", () => {
  let mechaDir: string;
  let mockRouter: MeshRouter;
  ...
  beforeEach(() => {
    mechaDir = mkdtempSync(join(tmpdir(), "mesh-test-"));
    mockRouter = {
      routeQuery: vi.fn().mockResolvedValue({ text: "ok", sessionId: undefined }),
    };
  });
  afterEach(() => { rmSync(mechaDir, { recursive: true, force: true }); vi.restoreAllMocks(); });

  it("sanitizes message before passing to router", async () => {
    const opts: MeshOpts = { mechaDir, botName: "sender", router: mockRouter };
    await handleMeshTool(opts, "mesh_query", {
      target: "receiver",
      message: "hello\x00\x01world",
    });

    expect(mockRouter.routeQuery).toHaveBeenCalledWith("sender", "receiver", "helloworld", undefined);
  });
});
