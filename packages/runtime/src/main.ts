...
const { app } = createServer({
  ...
  agentApiKey: (() => {
    try {
      ...
      if (!mechaDir) return undefined;
      const secret = readFileSync(join(mechaDir, "totp-secret"), "utf-8").trim();
