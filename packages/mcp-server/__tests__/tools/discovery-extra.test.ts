...
describe("mecha_list_bots remote edge cases", () => {
...
describe("mecha_bot_status remote edge cases", () => {
...
describe("mecha_discover with no expose config", () => {
  ...
  it("handles missing expose in config", async () => {
    ...
    (readBotConfig as ReturnType<typeof vi.fn>).mockReturnValue({ tags: ["test"] });
    const pm = {
      get: vi.fn(),
