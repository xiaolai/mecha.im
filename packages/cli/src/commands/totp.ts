...
/** Execute the TOTP show logic — display current secret. */
export function executeTotpShow(deps: CommandDeps): void {
  const secret = readTotpSecret(deps.mechaDir);
  if (!secret) {
    deps.formatter.error("No TOTP secret configured. Run 'mecha totp setup' first.");
    process.exitCode = 1;
...
export function registerTotpCommand(program: Command, deps: CommandDeps): void {
  ...
  totp
    .command("show")
    .description("Display the current TOTP secret")
    .action(async () => withErrorHandler(deps, async () => executeTotpShow(deps)));
