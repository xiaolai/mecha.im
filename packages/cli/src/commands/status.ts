...
export function registerStatusCommand(program: Command, deps: CommandDeps): void {
  program
    ...
    .action(async (opts: { port?: string }) => withErrorHandler(deps, async () => {
      ...
      /* v8 ignore start -- invalid port branch only hit with malformed CLI input */
      ...
      /* v8 ignore start -- JSON output path tested separately */
      ...
      /* v8 ignore start -- daemon status branches depend on live daemon process */
      if (daemonRunning) {
