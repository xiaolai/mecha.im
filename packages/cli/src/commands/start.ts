...
import type { PtySpawnFn } from "@mecha/process";
...
export function registerStartCommand(program: Command, deps: CommandDeps): void {
  program
    ...
    .action(async (opts: StartOpts) => withErrorHandler(deps, async () => {
      ...
      /* v8 ignore start -- node-pty is optional; terminal feature unavailable without it */
      let ptySpawnFn: PtySpawnFn | undefined;
      try {
        const { createNodePtySpawn } = await import("@mecha/process");
        ptySpawnFn = createNodePtySpawn() as PtySpawnFn;
