...
export function registerCostCommand(program: Command, deps: CommandDeps): void {
  program
    ...
    .argument("[bot]", "bot name (omit for all)")
    .action((bot?: string) =>
      withErrorHandler(deps, async () => {
        const dir = meterDir(deps.mechaDir);
        const result = bot
          ? queryCostForBot(dir, bot)
          : queryCostToday(dir);
