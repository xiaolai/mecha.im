...
describe("bot remove command", () => {
  ...
  it("removes multiple bots", async () => {
    mechaDir = mkdtempSync(join(tmpdir(), "mecha-bot-remove-"));
    ...
    await program.parseAsync(["node", "mecha", "bot", "remove", "alice", "bob"]);
    expect(existsSync(join(mechaDir, "alice"))).toBe(false);
    expect(existsSync(join(mechaDir, "bob"))).toBe(false);
