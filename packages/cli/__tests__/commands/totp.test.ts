...
import { executeTotpSetup, executeTotpVerify, executeTotpShow, executeTotpStatus } from "../../src/commands/totp.js";
...
describe("totp commands", () => {
  ...
  describe("executeTotpShow", () => {
    it("displays secret when configured", () => {
      writeFileSync(join(dir, "totp-secret"), "JBSWY3DPEHPK3PXP\n");
      const deps = makeDeps({ mechaDir: dir });
      executeTotpShow(deps);
      expect(deps.formatter.info).toHaveBeenCalledWith("JBSWY3DPEHPK3PXP");
    });

    it("errors when no secret configured", () => {
      const deps = makeDeps({ mechaDir: dir });
      executeTotpShow(deps);
      expect(deps.formatter.error).toHaveBeenCalledWith(
        expect.stringContaining("No TOTP secret"),
