...
describe("casa start command", () => {
  ...
  it("starts a stopped CASA from config", async () => {
    mechaDir = mkdtempSync(join(tmpdir(), "mecha-casa-start-"));
    ...
    await program.parseAsync(["node", "mecha", "casa", "start", "alice"]);
  ...
  it("errors if config is missing", async () => {
    ...
    await program.parseAsync(["node", "mecha", "casa", "start", "ghost"]);
    expect(deps.formatter.error).toHaveBeenCalledWith(expect.stringContaining("not found"));
    expect(process.exitCode).toBe(1);
  });

  it("errors if CASA is already running", async () => {
    mechaDir = mkdtempSync(join(tmpdir(), "mecha-casa-start-"));
