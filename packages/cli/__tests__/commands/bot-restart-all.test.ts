...
describe("bot restart-all command", () => {
  it("restarts all bots", async () => {
    mockBatch.mockResolvedValue(successResult(["alice", "bob"]));
    const deps = makeDeps();
    const program = createProgram(deps);
    program.exitOverride();

    await program.parseAsync(["node", "mecha", "bot", "restart-all"]);
  ...
  it("passes --force flag", async () => {
    ...
    await program.parseAsync(["node", "mecha", "bot", "restart-all", "--force"]);
  ...
  it("passes --idle-only flag", async () => {
    ...
    await program.parseAsync(["node", "mecha", "bot", "restart-all", "--idle-only"]);
  ...
  it("passes --dry-run and shows dry run message", async () => {
    ...
    await program.parseAsync(["node", "mecha", "bot", "restart-all", "--dry-run"]);
