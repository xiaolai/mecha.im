...
describe("casa remove command", () => {
  let mechaDir: string;
  afterEach(() => { if (mechaDir) rmSync(mechaDir, { recursive: true, force: true }); });

  it("stops running CASA and deletes directory", async () => {
    mechaDir = mkdtempSync(join(tmpdir(), "mecha-casa-remove-"));
    ...
    await program.parseAsync(["node", "mecha", "casa", "remove", "alice"]);
    expect(deps.processManager.stop).toHaveBeenCalledWith("alice");
    expect(existsSync(casaDir)).toBe(false);
  ...
  it("uses kill with --force", async () => {
    ...
    await program.parseAsync(["node", "mecha", "casa", "remove", "alice", "--force"]);
    expect(deps.processManager.kill).toHaveBeenCalledWith("alice");
    expect(deps.processManager.stop).not.toHaveBeenCalled();
    expect(existsSync(casaDir)).toBe(false);
  });

  it("deletes without stopping when CASA is already stopped", async () => {
    mechaDir = mkdtempSync(join(tmpdir(), "mecha-casa-remove-"));
    const casaDir = join(mechaDir, "alice");
    mkdirSync(casaDir, { recursive: true });
    writeFileSync(join(casaDir, "config.json"), JSON.stringify({ port: 7700, token: "t", workspace: "/ws" }));
  ...
  it("errors if CASA directory does not exist", async () => {
    mechaDir = mkdtempSync(join(tmpdir(), "mecha-casa-remove-"));
    ...
    await program.parseAsync(["node", "mecha", "casa", "remove", "ghost"]);
