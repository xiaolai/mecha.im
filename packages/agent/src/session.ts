...
/** Derive a hex signing key from the TOTP secret via HKDF-SHA256. */
export function deriveSessionKey(totpSecret: string): string {
  const key = hkdfSync("sha256", totpSecret, HKDF_SALT, HKDF_INFO, KEY_LENGTH);
...
export function verifySessionToken(
  ...
  /* v8 ignore start -- corrupt JWT payload fallback */
  } catch {
    return { valid: false };
  }
  /* v8 ignore stop */
