...
import { writeBotConfig, makeAcl } from "../../core/__tests__/test-utils.js";
...
import { botConfigure, checkBotBusy, getCachedSnapshot } from "@mecha/service";
const mockReadAuthProfiles = vi.mocked(readAuthProfiles);
const mockBotConfigure = vi.mocked(botConfigure);
...
describe("agent routes", () => {
  ...
  describe("health routes", () => {
    ...
    it("returns full system info on /node/info", async () => {
      const list: ProcessInfo[] = [
        { name: "a" as BotName, state: "running", port: 7700, workspacePath: "/ws" },
        { name: "b" as BotName, state: "stopped", workspacePath: "/ws2" },
      ];
      const app = Fastify();
      registerHealthRoutes(app, {
        nodeName: "bob",
      ...
      expect(body.botCount).toBe(1); // only "a" is running
      await app.close();
    });
  });
  ...
  describe("discover routes", () => {
    ...
    it("handles bot with no config file", async () => {
      mechaDir = mkdtempSync(join(tmpdir(), "agent-discover-"));
      const list: ProcessInfo[] = [
        { name: "ghost" as BotName, state: "running", port: 7700, workspacePath: "/ws" },
      ];
      const app = Fastify();
    ...
    it("handles malformed tags in config", async () => {
      ...
      const body = res.json();
      expect(body).toHaveLength(1);
      // readBotConfig normalizes invalid tags/expose to undefined → discover treats as []
      expect(body[0].tags).toEqual([]);
      expect(body[0].expose).toEqual([]);
      await app.close();
    });
    ...
    it("filters by both tag and capability", async () => {
      ...
      const list: ProcessInfo[] = [
        ...
        { name: "writer" as BotName, state: "running", port: 7701, workspacePath: "/b" },
      ];
      writeBotConfig(mechaDir, "coder", {
        port: 7700, token: "t", workspace: "/a", tags: ["code"], expose: ["query"],
      });
      writeBotConfig(mechaDir, "writer", {
  ...
  describe("bot routes", () => {
    beforeEach(() => {
      mechaDir = mkdtempSync(join(tmpdir(), "agent-bots-"));
    });
    ...
    it("returns enriched bot list with projection", async () => {
      const list: ProcessInfo[] = [
        { name: "a" as BotName, state: "running", port: 7700, workspacePath: "/home/user/project", token: "secret", pid: 1234 },
        { name: "b" as BotName, state: "stopped", workspacePath: "/ws2" },
      ];
      writeBotConfig(mechaDir, "a", {
        port: 7700, token: "tok", workspace: "/home/user/project",
        model: "claude-sonnet-4-20250514", tags: ["coder"],
      });
      const app = Fastify();
      registerBotRoutes(app, makePm(list), mechaDir);
      await app.ready();
    ...
    it("includes costToday when snapshot has data", async () => {
      ...
      mockGetSnapshot.mockReturnValueOnce({
        ...
        byBot: { a: { today: { ...emptySummary, costUsd: 2.50 }, thisMonth: emptySummary } },
        byAuth: {}, byTag: {},
      });
      const app = Fastify();
      registerBotRoutes(app, makePm(list), mechaDir);
    ...
    describe("POST /bots/:name/start", () => {
      it("starts a stopped bot from config", async () => {
        writeBotConfig(mechaDir, "alice", {
          port: 7700, token: "tok", workspace: "/ws",
        });
        const pm = makePm([
          { name: "alice" as BotName, state: "stopped", workspacePath: "/ws" },
        ]);
        ...
        const res = await app.inject({ method: "POST", url: "/bots/alice/start" });
        expect(res.statusCode).toBe(200);
      ...
      it("returns 409 when bot is already running", async () => {
        ...
        expect(res.json().code).toBe("BOT_ALREADY_RUNNING");
        await app.close();
      });
      ...
      it("returns 404 when no config exists", async () => {
        ...
        const res = await app.inject({ method: "POST", url: "/bots/ghost/start" });
      ...
      it("returns 400 for invalid name", async () => {
        ...
        const res = await app.inject({ method: "POST", url: "/bots/INVALID/start" });
        expect(res.statusCode).toBe(400);
        await app.close();
      });
    });

    describe("POST /bots/:name/restart", () => {
      it("restarts a running bot that is not busy", async () => {
        writeBotConfig(mechaDir, "alice", {
          port: 7700, token: "tok", workspace: "/ws",
        });
        const pm = makePm([
        ...
        const res = await app.inject({ method: "POST", url: "/bots/alice/restart" });
        expect(res.statusCode).toBe(200);
        expect(res.json()).toEqual({ ok: true, name: "alice", port: 7700 });
      ...
      it("returns 409 when bot is busy and force not set", async () => {
        writeBotConfig(mechaDir, "alice", {
          port: 7700, token: "tok", workspace: "/ws",
        });
        const pm = makePm([
      ...
      it("starts a stopped bot via restart", async () => {
        writeBotConfig(mechaDir, "alice", {
          port: 7700, token: "tok", workspace: "/ws",
        });
        const pm = makePm([
      ...
      it("returns 404 when no config exists", async () => {
        ...
        const res = await app.inject({ method: "POST", url: "/bots/ghost/restart" });
        expect(res.statusCode).toBe(404);
        await app.close();
      });
    });

    describe("POST /bots/:name/stop (task check)", () => {
      it("stops a non-busy bot", async () => {
        ...
        const res = await app.inject({ method: "POST", url: "/bots/alice/stop" });
        expect(res.statusCode).toBe(200);
        expect(pm.stop).toHaveBeenCalledWith("alice");
        await app.close();
      });
      ...
      it("stops busy bot when force=true", async () => {
        const pm = makePm([
          { name: "alice" as BotName, state: "running", port: 7700, workspacePath: "/ws" },
        ]);
        mockCheckBusy.mockResolvedValue({
    ...
    describe("PATCH /bots/:name/config", () => {
      ...
      it("returns 409 when busy bot + restart without force", async () => {
        const pm = makePm([
          { name: "alice" as BotName, state: "running", port: 7700, workspacePath: "/ws" },
        ]);
        mockCheckBusy.mockResolvedValue({
      ...
      it("skips restart when bot is stopped", async () => {
        const pm = makePm([
          { name: "alice" as BotName, state: "stopped", workspacePath: "/ws" },
        ]);
        const app = Fastify();
  ...
  describe("routing routes", () => {
    it("validates bot name format", async () => {
      ...
      const res = await app.inject({
        method: "POST",
        url: "/bots/INVALID_NAME/query",
        headers: { "x-mecha-source": "coder" },
        payload: { message: "hello" },
      });
      expect(res.statusCode).toBe(400);
      expect(res.json().error).toContain("Invalid bot name");
      await app.close();
    });

    it("returns 502 on forwarding failure", async () => {
      mechaDir = mkdtempSync(join(tmpdir(), "agent-routing-"));
      writeBotConfig(mechaDir, "target", {
