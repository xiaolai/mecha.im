...
function createMockPty(): IPty & { _emitExit: (code: number) => void } {
  const emitter = new EventEmitter();
  return {
    pid: 1234, cols: 80, rows: 24, process: "claude", handleFlowControl: false,
  ...
  } as unknown as IPty & { _emitExit: (code: number) => void };
}
