...
export function wrapLinux(
  ...
  // Tmpfs for /tmp BEFORE user binds — user bind mounts under /tmp will overlay
  // on top of the tmpfs, making workspace paths like /tmp/project accessible.
  args.push("--tmpfs", "/tmp");

  // Read-only binds (after system mounts so they can overlay)
  for (const p of profile.readPaths) {
    args.push("--ro-bind", p, p);
  }

  // Read-write binds (after tmpfs so /tmp/* workspaces are visible)
