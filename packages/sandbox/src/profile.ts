...
/** Options for generating a sandbox profile from bot config. */
...
export function profileFromConfig(opts: ProfileFromConfigOpts): SandboxProfile {
  ...
  // Allow shell and POSIX utilities needed by sandbox enforcement hooks (R9-001).
  // Hooks (sandbox-guard.sh, bash-guard.sh) call bash, node (via process.execPath above),
  // and standard utilities for path canonicalization and pattern matching.
  const hookUtilities = [
    "/bin/bash", "/bin/sh", "/usr/bin/env",
    "/usr/bin/realpath", "/usr/bin/dirname", "/usr/bin/basename",
    "/usr/bin/grep", "/usr/bin/sed", "/usr/bin/cat",
  ...
  for (const util of hookUtilities) {
    if (existsSync(util)) allowedProcesses.push(util);
  }
