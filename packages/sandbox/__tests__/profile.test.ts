...
describe("profileFromConfig", () => {
  ...
  it("includes shell utilities in allowed processes for hook execution (R9-001)", () => {
    const profile = profileFromConfig({
      config: baseConfig,
      botDir: "/mecha/alice",
      mechaDir: "/mecha",
    ...
    // At least bash or sh should be present on any POSIX system
    const hasBash = profile.allowedProcesses.some((p) => p.endsWith("/bash") || p.endsWith("/sh"));
