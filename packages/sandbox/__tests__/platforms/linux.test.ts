...
describe("wrapLinux", () => {
  ...
  it("includes read-only binds for read paths", () => {
    ...
    // Find the --ro-bind for the specific read path (not system paths)
    const pairs: [string, string][] = [];
    for (let i = 0; i < args.length - 2; i++) {
      if (args[i] === "--ro-bind") pairs.push([args[i + 1]!, args[i + 2]!]);
    }
    expect(pairs).toContainEqual(["/usr/bin/node", "/usr/bin/node"]);
  });
