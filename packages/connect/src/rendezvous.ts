...
/** Options for creating a single-server rendezvous client. */
...
/** Create a RendezvousClient that connects to a single rendezvous server with auto-reconnect. */
export function createRendezvousClient(opts: CreateRendezvousClientOpts): RendezvousClient {
  ...
  const client: RendezvousClient = {
    ...
    close(): void {
      closedByUser = true;
      registered = false;
      /* v8 ignore start -- reconnect timer cleanup requires active reconnect */
      if (reconnectTimer) {
