...
import { batchCasaAction } from "../src/casa-batch.js";
...
const BOB: ProcessInfo = { name: "bob" as CasaName, state: "running", pid: 2, port: 7701, workspacePath: "/ws2" };
const CHARLIE: ProcessInfo = { name: "charlie" as CasaName, state: "stopped", workspacePath: "/ws3" };

describe("batchCasaAction", () => {
  let mechaDir: string;
  afterEach(() => {
  ...
  describe("stop action", () => {
    it("stops all running CASAs, skips stopped", async () => {
      mechaDir = mkdtempSync(join(tmpdir(), "batch-stop-"));
      const pm = makePm([ALICE, BOB, CHARLIE]);
      const result = await batchCasaAction({ pm, mechaDir, action: "stop" });
    ...
    it("skips busy CASAs silently with idleOnly", async () => {
      mechaDir = mkdtempSync(join(tmpdir(), "batch-stop-"));
      ...
      const result = await batchCasaAction({ pm, mechaDir, action: "stop", idleOnly: true });
    ...
    it("force bypasses busy check", async () => {
      mechaDir = mkdtempSync(join(tmpdir(), "batch-stop-"));
      const pm = makePm([ALICE, BOB]);
      const result = await batchCasaAction({ pm, mechaDir, action: "stop", force: true });
    ...
    it("dryRun returns status without executing", async () => {
      ...
      const pm = makePm([ALICE, BOB]);
      const result = await batchCasaAction({ pm, mechaDir, action: "stop", dryRun: true });
    ...
    it("returns empty results when no CASAs exist", async () => {
      mechaDir = mkdtempSync(join(tmpdir(), "batch-stop-"));
      const pm = makePm([]);
      const result = await batchCasaAction({ pm, mechaDir, action: "stop" });
    ...
    it("fails busy CASAs by default (without idleOnly or force)", async () => {
      mechaDir = mkdtempSync(join(tmpdir(), "batch-stop-"));
    ...
    it("filters by names when provided", async () => {
      mechaDir = mkdtempSync(join(tmpdir(), "batch-stop-"));
      const pm = makePm([ALICE, BOB]);
      const result = await batchCasaAction({ pm, mechaDir, action: "stop", names: ["alice"] });
    ...
    it("clamps concurrency to minimum 1", async () => {
      mechaDir = mkdtempSync(join(tmpdir(), "batch-stop-"));
      const pm = makePm([ALICE]);
      const result = await batchCasaAction({ pm, mechaDir, action: "stop", concurrency: 0 });
  ...
  describe("restart action", () => {
    it("restarts all CASAs from config", async () => {
      ...
      const result = await batchCasaAction({ pm, mechaDir, action: "restart" });
    ...
    it("uses kill when force is set", async () => {
      ...
      await batchCasaAction({ pm, mechaDir, action: "restart", force: true });
    ...
    it("spawns stopped CASAs without stopping first", async () => {
      mechaDir = mkdtempSync(join(tmpdir(), "batch-restart-"));
    ...
    it("dryRun for restart checks config existence", async () => {
      ...
      const result = await batchCasaAction({ pm, mechaDir, action: "restart", dryRun: true });
