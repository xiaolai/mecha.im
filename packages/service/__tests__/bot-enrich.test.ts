...
import { enrichBotInfo, buildEnrichContext } from "../src/bot-enrich.js";
...
function makeSnapshot(byBot: Record<string, { today: CostSummary; thisMonth: CostSummary }> = {}): HotSnapshot {
  return {
...
describe("enrichBotInfo", () => {
  ...
  it("merges ProcessInfo and config fields", () => {
    ...
    try {
      ...
      const result = enrichBotInfo(makeInfo(), ctx);
  ...
  it("never leaks token field", () => {
    ...
    try {
      ...
      const ctx = buildEnrichContext(dir, null, ["alice"]);
      const result = enrichBotInfo(info, ctx);
  ...
  it("pre-loads all configs in buildEnrichContext", () => {
    ...
    try {
      ...
      const alice = enrichBotInfo(makeInfo(), ctx);
      const bob = enrichBotInfo(makeInfo({ name: "bob" as BotName, workspacePath: "/ws2" }), ctx);
