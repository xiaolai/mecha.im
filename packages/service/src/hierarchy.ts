...
/**
 * Build a tree of bots based on workspace path nesting.
 * A bot is a child of another if its workspace is a subdirectory.
 */
export function buildHierarchy(bots: FindResult[]): HierarchyNode[] {
...
export function flattenHierarchy(roots: HierarchyNode[]): { bot: FindResult; depth: number }[] {
  const result: { bot: FindResult; depth: number }[] = [];

  function walk(nodes: HierarchyNode[]): void {
    for (const node of nodes) {
      result.push({ bot: node.bot, depth: node.depth });
      walk(node.children);
    }
  }
