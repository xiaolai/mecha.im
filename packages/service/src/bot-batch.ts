...
export async function batchBotAction(opts: BatchActionOpts): Promise<BatchResult> {
  ...
  if (dryRun) {
    for (const c of candidates) {
      ...
      if (c.busy && !opts.force && !opts.idleOnly) {
        status = "failed";
        error = `bot has ${c.activeSessions} active session(s)`;
      /* v8 ignore start -- idle-only skip in dry-run */
      } else if (c.busy && opts.idleOnly) {
        status = "skipped_busy";
      /* v8 ignore stop */
