...
import { isValidName, readCasaConfig } from "@mecha/core";
import type { ProcessManager, ProcessInfo } from "@mecha/process";
import { checkCasaBusy } from "./task-check.js";
...
export interface BatchActionOpts {
  ...
  action: "stop" | "restart";
  force?: boolean;
  /** Skip busy CASAs silently (counted as skipped). Without this, busy CASAs are counted as failed. */
  idleOnly?: boolean;
  dryRun?: boolean;
  concurrency?: number;
  /** Limit operation to specific CASA names. When omitted, targets all CASAs. */
  names?: string[];
...
/**
 * Run a batch stop/restart across all CASAs.
 * Returns per-CASA results with a summary.
 *
 * Busy-check semantics:
 * - `force`: bypass busy check entirely
 * - `idleOnly`: skip busy CASAs silently (counted as skipped, not failed)
 * - default: busy CASAs are counted as failed (operation refused)
 *
 * Note: TOCTOU between busy check and stop/kill is inherent to the domain.
 * A CASA can become busy after the check. This is acceptable — the alternative
 * (holding a lock) would be worse for a local-first system.
 */
export async function batchCasaAction(opts: BatchActionOpts): Promise<BatchResult> {
  ...
  for (let i = 0; i < toCheck.length; i += concurrency) {
    ...
    const checks = await Promise.all(
      chunk.map((info) => checkCasaBusy(pm, info.name as CasaName)),
    );
  ...
  if (dryRun) {
    for (const c of candidates) {
      ...
      if (c.busy) {
        ...
        const config = readCasaConfig(join(mechaDir, c.info.name));
        if (!config) {
          status = "failed";
...
async function executeSingle(
  ...
  const name = info.name as CasaName;
