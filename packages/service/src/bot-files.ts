...
/** Directory entry returned by bot file listing. */
 ...
 */
/** Resolve the HOME directory for a bot (custom or default botDir). */
 ...
 */
/** List files in a bot's HOME directory (path-traversal safe). */
export async function listBotDir(homeDir: string, relPath: string): Promise<DirEntry[]> {
  ...
  try {
    ...
    /* v8 ignore start -- re-throw for unexpected readdir errors */
    throw err;
    /* v8 ignore stop */
  }
 ...
 */
/** Read a file from a bot's HOME directory (path-traversal safe, size-limited). */
export async function readBotFile(homeDir: string, relPath: string): Promise<string> {
  ...
  try {
    ...
    /* v8 ignore start -- re-throw for unexpected open errors */
    throw err;
    /* v8 ignore stop */
  }
  try {
    const info = await fh.stat();
    /* v8 ignore start -- non-file stat guard (O_NOFOLLOW already rejects symlinks) */
 ...
 */
/** Write a file to a bot's HOME directory (path-traversal safe, markdown-only). */
export async function writeBotFile(homeDir: string, relPath: string, content: string): Promise<void> {
  ...
  /* v8 ignore start -- symlink parent guard requires symlink race */
  ...
  try {
    ...
    /* v8 ignore start -- re-throw for unexpected write open errors */
    throw err;
    /* v8 ignore stop */
  }
  try {
...
/** Thrown when a requested bot file does not exist. */
...
/** Thrown when a write targets a non-markdown file. */
...
/** Thrown when a requested file exceeds the size limit. */
