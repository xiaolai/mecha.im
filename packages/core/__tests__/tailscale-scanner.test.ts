...
describe("parseTailscaleStatus", () => {
  ...
  it("handles missing Self and Peer fields", () => {
    expect(parseTailscaleStatus({})).toEqual([]);
  });

  it("uses IP as hostname when HostName is missing", () => {
    const json = {
      Peer: {
        "nodekey:abc": { TailscaleIPs: ["100.100.1.5"], Online: true },
      },
    };
    const peers = parseTailscaleStatus(json);
    expect(peers).toHaveLength(1);
    expect(peers[0]).toEqual({ ip: "100.100.1.5", hostname: "100.100.1.5" });
  });
});
