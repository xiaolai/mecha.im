...
describe("AclEngine", () => {
  ...
  describe("orchestration capabilities", () => {
    it("grants and checks bus_publish capability", () => {
      exposeMap["broker"] = ["bus_publish"];
      const acl = makeEngine();
      acl.grant("writer", "broker", ["bus_publish"]);
      expect(acl.check("writer", "broker", "bus_publish")).toEqual({ allowed: true });
    });

    it("grants and checks workflow_manage capability", () => {
      exposeMap["scheduler"] = ["workflow_manage"];
      const acl = makeEngine();
      acl.grant("meta", "scheduler", ["workflow_manage"]);
      expect(acl.check("meta", "scheduler", "workflow_manage")).toEqual({ allowed: true });
    });

    it("grants and checks meta capability", () => {
      exposeMap["system"] = ["meta"];
      const acl = makeEngine();
      acl.grant("ceo", "system", ["meta"]);
      expect(acl.check("ceo", "system", "meta")).toEqual({ allowed: true });
    });

    it("grants multiple orchestration capabilities at once", () => {
      exposeMap["hub"] = ["bus_publish", "bus_subscribe", "bus_queue", "gateway_outbound"];
      const acl = makeEngine();
      acl.grant("agent", "hub", ["bus_publish", "bus_subscribe", "bus_queue", "gateway_outbound"]);
      expect(acl.check("agent", "hub", "bus_publish")).toEqual({ allowed: true });
      expect(acl.check("agent", "hub", "bus_subscribe")).toEqual({ allowed: true });
      expect(acl.check("agent", "hub", "bus_queue")).toEqual({ allowed: true });
      expect(acl.check("agent", "hub", "gateway_outbound")).toEqual({ allowed: true });
      expect(acl.check("agent", "hub", "workflow_run")).toEqual({ allowed: false, reason: "no_connect" });
    });
  });
});
