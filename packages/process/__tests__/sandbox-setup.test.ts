...
describe("sandbox-setup", () => {
  ...
  describe("shared HOME (--home)", () => {
    ...
    it("writes settings.json and credentials for non-shared HOME (default)", () => {
      prepareBotFilesystem(makeOpts());
    ...
    it("writes settings.json to shared HOME when no pre-existing files", () => {
      const sharedHome = mkdtempSync(join(tmpdir(), "mecha-shared-new-"));
