...
import { isPidAlive, waitForChildExit, waitForPidExit, waitForPortFree } from "./process-lifecycle.js";
...
/** Create a ProcessManager that manages bot process lifecycle within a mechaDir. */
export function createProcessManager(opts: CreateProcessManagerOpts): ProcessManager {
  ...
  async function stopBot(name: BotName): Promise<void> {
    ...
    if (!lp) {
      ...
      writeState(_botDir(name), state);
      _updateDiscoveryIndex();
      // Wait for the port to be fully released by the OS (prevents EADDRINUSE on restart)
      if (state.port) await waitForPortFree(state.port, 3_000);
    ...
    /* v8 ignore stop */
    // Wait for the port to be fully released by the OS (prevents EADDRINUSE on restart).
    // Read port from state — only wait for real ports in the bot port range.
    const finalState = readState(_botDir(name));
    /* v8 ignore start -- port release wait requires real port lifecycle */
    if (finalState?.port && finalState.port >= 7700 && finalState.port <= 7799) {
      await waitForPortFree(finalState.port, 3_000);
    }
    /* v8 ignore stop */
    });
  }
