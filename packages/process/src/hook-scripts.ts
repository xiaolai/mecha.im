 ...
 * Merges hooks into existing settings.json (preserves permissions and other fields).
...
export function writeHookScripts(claudeDir: string, hooksDir: string): void {
  ...
  // Merge with existing settings.json to preserve permissions and other fields
  const settingsPath = join(claudeDir, "settings.json");
  let existing: Record<string, unknown> = {};
  if (existsSync(settingsPath)) {
    try {
      existing = JSON.parse(readFileSync(settingsPath, "utf-8")) as Record<string, unknown>;
    /* v8 ignore start -- corrupt settings.json fallback */
    } catch { /* corrupt file — overwrite */ }
    /* v8 ignore stop */
  }

  // Ensure runtime MCP tools are allowed (merge with existing permissions)
  const mcpToolPerms = [
  ...
  const mergedAllow = [...new Set([...existingAllow, ...mcpToolPerms])];
