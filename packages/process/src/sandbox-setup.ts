...
/** Options for preparing a bot's sandboxed filesystem layout. */
...
/** Result of preparing a bot's filesystem, including paths and environment variables. */
...
function seedClaudeBinSymlink(homeDir: string): void {
  ...
  /* v8 ignore start -- claude binary detection varies per machine */
...
/** Create the bot directory structure, write config, seed credentials, and build child env vars. */
export function prepareBotFilesystem(opts: BotFilesystemOpts): BotFilesystemResult {
  ...
  /* v8 ignore start -- relative home path rejected at config validation layer */
