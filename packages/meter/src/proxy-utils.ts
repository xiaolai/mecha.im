...
/** Shared state for the meter proxy: pricing, counters, budgets, and bot registry */
...
/* v8 ignore start -- test-only helper */
...
function maybeReloadBudgets(ctx: ProxyContext): void {
  ...
  try {
    ...
    if (!_budgetStatCache || mtimeMs !== _budgetStatCache.mtimeMs) {
      /* v8 ignore start -- budget reload on mtime change */
    ctx.budgets = readBudgets(ctx.meterDir);
    /* v8 ignore stop */
    }
    /* v8 ignore start -- budget stat cache update */
