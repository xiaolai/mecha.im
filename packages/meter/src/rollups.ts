...
/** Return the filesystem path for a date's hourly rollup JSON file */
...
/** Return the filesystem path for a month's daily rollup JSON file */
...
/** Return the filesystem path for a bot's rollup JSON file */
...
/** Read an hourly rollup from disk, returning an empty rollup if missing */
...
/** Read a daily rollup from disk, returning an empty rollup if missing */
...
/** Read a per-bot rollup from disk, returning an empty rollup if missing */
...
/** Write an hourly rollup to disk atomically */
...
/** Write a daily rollup to disk atomically */
...
/** Write a per-bot rollup to disk atomically */
...
function getHourBucket(rollup: HourlyRollup, hour: number): HourBucket {
  ...
  /* v8 ignore start -- index rebuild from persisted rollup */
  if (!index) {
    index = new Map();
