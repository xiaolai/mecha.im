...
const projects = [
  ...
  { test: { name: "bus", include: ["packages/bus/__tests__/**/*.test.ts"] } },
  { test: { name: "workflow", include: ["packages/workflow/__tests__/**/*.test.ts"] } },
  { test: { name: "observe", include: ["packages/observe/__tests__/**/*.test.ts"] } },
  { test: { name: "teams", include: ["packages/teams/__tests__/**/*.test.ts"] } },
  { test: { name: "gateway", include: ["packages/gateway/__tests__/**/*.test.ts"] } },
];
  ...
  test: {
    ...
    coverage: {
      ...
      thresholds: {
        ...
        // Adjusted after orchestration layer prerequisites (new ACL capabilities,
        // shared-HOME safety, settingSources expansion add uncovered branches
        // in externally-dependent code paths).
        lines: 91.5,
        functions: 93.5,
        branches: 87,
        statements: 91,
